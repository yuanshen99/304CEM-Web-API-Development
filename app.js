    ...
    .then(response => {
      if (response.length) {
        var mailOptions = {
          ...
          html: '<h1>Welcome to weather api!</h1><p>Your api key is ' + uuidAPIKey.toAPIKey(response[0].key) + ' and registered email is '+ email +'<p>Use the following address to access to the api:</p>https://theweatherapi.herokuapp.com/getweather?location=${locationname}&apikey=${apikey}&userid=${registeredemail}</p>'
        };
...
app.get('/generatekey', (req, res) => {
  ...
  .then(response => {
    var mailOptions = {
      ...
      to: email,
      subject: 'Api Key for weather',
      html: '<h1>Welcome to weather api!</h1><p>Your api key is ' + create.apiKey + ' and registered email is '+ email +'<p>Use the following address to access to the api:</p>https://theweatherapi.herokuapp.com/getweather?location=${locationname}&apikey=${apikey}&userid=${registeredemail}</p>'
    };
